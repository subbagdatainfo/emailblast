...
$sql = "SELECT post_title, guid FROM test.wp_posts WHERE post_date_gmt LIKE '$hari%'
		UNION 
		SELECT post_title, guid FROM test.wp_2_posts WHERE post_date_gmt LIKE '$hari%'
		UNION 
		SELECT post_title, guid FROM test.wp_4_posts WHERE post_date_gmt LIKE '$hari%'";
...
$result = $conn->query($sql);
    ...
    // output data of each row
    	while($row = $result->fetch_assoc()) {
        	$data = "<a href='".$row['guid']."'".">".$row["post_title"]."</a>"."<br>";
        	$simpandata[]=$data;
    	}
	} else {
    echo "0 results";
